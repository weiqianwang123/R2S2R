...
from r2s2r.sim.isaaclab.scene import build_scene_cfg  # noqa: E402
...
def main() -> None:
    ...
    for cam in spec.cameras.values():
        ...
        if key not in scene.keys():
            continue
        rgb = scene[key].data.output["rgb"][0, ..., :3].cpu().numpy()
        depth = scene[key].data.output["distance_to_image_plane"][0, ..., 0]
        rendered = (depth.cpu().numpy() < 50.0)[..., None]  # background is inf
