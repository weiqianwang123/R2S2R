...
from r2s2r.io.droid import align_steps_to_video, load_droid_episode, static_step_range
...
def test_load_droid_episode(droid_episode, tmp_path):
    """Frames, calibration and robot state end up in a reloadable capture."""
    ...
    assert set(capture.cameras) == set(SERIALS.values())
    assert len(capture.frames) == 3 * len(range(0, CLOSE_STEP, 2))
    ...
    assert capture.metadata["calibration_source"][SERIALS["ext2"]] == "trajectory.h5"
